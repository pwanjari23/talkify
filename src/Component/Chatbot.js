...
import { IoChatboxSharp, IoMenuSharp, IoMoonSharp, IoSunnySharp, IoCopyOutline } from "react-icons/io5";
...
const Chatbot = () => {
    ...
    const copyToClipboard = (index) => {
        const textToCopy = messages[index].text.replace(/^[`*]+|[`*]+$/g, '');
        ...
        <div className={`flex items-center justify-center min-h-screen p-4 ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-black'}`}>
            {showHiMessage && (
                <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 p-4 z-50">
                    <img src={chatbot} className="text-2xl w-48 h-48" />
                </div>
            )}
            ...
            <div className="flex flex-col w-full max-w-3xl rounded-lg h-full">
                ...
                <div className='fixed top-2 right-6  flex items-center'>
                ...
                <div className={`flex-1 overflow-y-auto p-6 ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-black'}`}>
                    {messages.map((message, index) => (
                        <div
                            key={index}
                            className={`my-2 p-2 rounded-lg flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'}`}
                        ...
                        >
                            <div className={`relative rounded-lg p-4 mb-6 lg:max-w-[75%] sm:max-w-[75%] break-words overflow-auto ${message.sender === 'user' ? (darkMode ? 'bg-gray-700 text-white' : 'bg-blue-500 text-white') : darkMode ? 'bg-gray-800 text-white' : 'bg-gray-200 text-black'}`}>
                                ...
                                {hoveredMessageIndex === index && message.sender !== 'user' && (
                                    <IoCopyOutline
                                        className="absolute top-1 right-1 text-md cursor-pointer"
                    ...
                    {loading && (
                        <div className="my-2 p-2 rounded-lg flex justify-start">
                            <div className={`rounded-lg p-4 mb-6 max-w-[75%] sm:max-w-full break-words ${darkMode ? 'text-white' : 'text-black'}`}>
